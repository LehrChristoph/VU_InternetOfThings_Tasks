...
static char *get_mqtt_payload(temp_value_t* temp)
{
    ...
    if (temp != 0)
    {
        snprintf(payload, 70, "{temperature: %f, timestamp: %llu}", temp->temperature, temp->timestamp );
    }
    else {
        snprintf(payload, 70, "no temperature value availible");
    }
...
static int publish(struct mqtt_client *client, enum mqtt_qos qos, temp_value_t* temp)
{
...
int mqtt_handler_publish(temp_value_t* temp)
{
	int rc;
